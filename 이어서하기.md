...
**마지막 갱신**: 2026-07-24
**현재 위치**: T1~T7·T9·**T10 완료** + **프론트(apps/web, 목 데이터·shadcn 다크)** → **다음 작업: T11 (수익률·시뮬레이션 — 거래비용·슬리피지·달러/원화 병기)**

> 크리티컬 패스: T9(기준가)✅→T10(자동청산)✅→T11(수익률)→T12(지표, 골든 0.01%p).
> 지금 프론트 숫자는 전부 목 데이터 — T12~T13이 있어야 진짜 성과·랭킹이 채워진다.
...
## 3. 다음 작업: T11 — 수익률·시뮬레이션 (PERF-005~007, PERF-011)

**위치**: `batch/src/signals_batch/performance/` — 수익률 계산(순수) + 배치, positions에 비용·수익률 채움

**앞선 작업 요약 (T10, 2026-07-24 완료)**:
...
- **미검증**: 라이브 DB E2E는 아직 안 함(T9는 실데이터 E2E까지 했음). 순수 로직 단위테스트만 통과.
- **경계(T7/T11 소관) 발견**: 수동 EXIT는 `exit_signal_id`만 세팅되고 `status`=OPEN·`exit_price`=NULL로 남음
  (pairing.service.ts closePosition). DISCRETIONARY 청산가(PERF-004: 익일시가/VWAP) 산출·CLOSED 전환이 미완.

**T11 할 일** (SIG-019/020 청산가는 T10에서 확정됨 → 여기선 비용·수익률):
1. 거래비용(PERF-005): 국내 매수 0.015%/매도 0.215%(거래세 포함), 미국 매수 0.07%/매도 0.0708%
2. 슬리피지(PERF-006): 시총 구간별 base × liquidity_multiplier → **시가총액 데이터 필요**(T2 후속 보강 미완, 우선 대응 결정 필요)
3. gross/net_return 산출 + 달러·원화 병기(PERF-007, 미국주식 환율 entry_fx/exit_fx)
4. 가상 포트폴리오 시뮬레이션(PERF-011), 자금부족 스킵 기록(검수 2.13)
5. **완료 기준**: positions.net_return 채워짐, 검수 2.7~2.9·2.13. **골든 대조는 T12**(오차 0.01%p).

크리티컬 패스: **T2→T9→T10→T11→T12** (시장데이터 → 기준가 → 자동청산 → 수익률 → 골든 오차 0.01%p).
