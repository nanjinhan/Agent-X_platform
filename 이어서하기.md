...
**마지막 갱신**: 2026-07-24
**현재 위치**: T1~T7·T9 완료 + **프론트(apps/web, 목 데이터·shadcn 다크)** → **다음 작업: T10 (자동 청산 판정 — SIG-019/020 갭 처리)**
...
## 3. 다음 작업: T7 — 발행 13단계 검증 + 진입-청산 페어링

**위치**: `apps/signal-service/src/publish/`(검증 확장), `src/pairing/`(신규)

**할 일** (SYS-013 전체, SIG-010~011, SIG-018, REG-013):
1. 발행 검증 나머지: 발행 시간대(SIG-010, 동시호가 금지), 장중 발행 조건(SIG-011),
   종목 유효성·시총/거래대금 제한(REG-013 — instruments 테이블 조회), 금칙어(domain의 findProhibitedTerm),
   일일 발행 한도, 동일 종목 미결제 중복 확인, market_session 판정(현재 'CLOSED' 하드코딩)
2. valid_until 계산·저장 (SIG-003: 기본 3거래일, trading_calendar 사용) — **해시에 이미 자리 있음(null)**
3. 공급자 인증: core-api가 발급한 JWT 검증(공유 시크릿) + 에이전트 소유권 확인
4. pairing: EXIT 시그널 → entry_signal_id 검증(SIG-018: 1:1, 이미 청산된 것 재청산 금지) → positions 행 생성
5. **완료 기준**: ENTRY→EXIT 발행 시 positions 페어 생성, 발행 금지 시간대 422, 검수 1.x 유지

**T7 이후**: T8은 사실상 T6에서 선구현됨(공개 검증 API) → T9~(성과 엔진, Python)로 직행 가능.
