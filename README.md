...
1. **[이어서하기.md](./이어서하기.md) ← 여기부터.** "어디까지 했고, 다음에 뭘 해야 하는지"의 단일 출처.
   맨 위에 현재 위치(완료 태스크 → 다음 태스크)가 적혀 있다.
2. **[docs/TASKS.md](./docs/TASKS.md)** — 전체 구현 계획 T1~T30과 각 상태(⬜/🔄/✅).
   이어서하기가 "지금"이라면 이건 "전체 지도".
3. **[아래 빠른 시작](#빠른-시작-환경-셋업)** 으로 로컬 환경(DB·빌드)을 띄운다.
4. 그다음 이어서하기의 "다음 작업" 섹션이 시키는 태스크를 시작한다.

> 진행 상태의 진실은 **이어서하기.md와 docs/TASKS.md**에 있다. 대화 기록이나 기억이 아니라 이 두 파일을 믿을 것.
> 깃허브에 푸시할 때는 반드시 이어서하기.md를 먼저 갱신한다.
...
> 데이터 소스는 정식 계약 전까지 무료 소스(네이버·KIND·야후)로 우회 구현되어 있다 — 한계는 이어서하기.md 참조.
